...
The Original Code is Header for Rigid Probe HIO (probe.h).
...
//
//	Rigid Probe as a Haptic Interface Object 
//
//
class	RigidProbeHIO : public HapticInterfaceObject {
public:
	// Constructors
	RigidProbeHIO(XMLNode * simObjectNode);
	~RigidProbeHIO()
	{
		// for a probe, its boundary is its geometry, so we can't let it try to delete both in the SIMObject destructor
		if (boundary)
		{
			boundary = NULL;
		}
...
//	Vector<Real> GetPosition(){ return HI_position; };
//	Vector<Real> GetVelocity(){ return Vector<Real>(3,0.0); };
...
// Boundary for RigidProbeHIO 
class RigidProbeHIOBoundary : public HapticInterfaceObjectBoundary {
public:
	void SetNullModel() 
	{		
	...
	void ResetBoundaryCondition(void) { SetNullModel(); }
...
class RigidProbeHIODomain : public HapticInterfaceObjectDomain {
};
