...
float min_dist, float max_dist, const OPC_Point& view_point, CullModeCallback callback, void* user_data)
{
	struct Local
	{
		struct CullData
		...
		static void RenderCullingCallback(const CollisionFace& hit, void* user_data)
		{
			...
			// Discard face if hit OPC_Point is smaller than min limit. This mainly happens when the face is in front
