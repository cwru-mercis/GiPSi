 ...
 *	box OPC_Pointers and primitive OPC_Pointers have been made implicit, hence packing 3 OPC_Pointers in one.
 *
 ...
 *			- data (32-bits value)
 *
 *	if data's LSB = 1 =>	remaining bits are a primitive OPC_Pointer
 *	else					remaining bits are a P-node OPC_Pointer, and N = P + 1
 *
...
static void _BuildCollisionTree(AABBCollisionNode* linear, const udword box_id, udword& current_id, const AABBTreeNode* current_node)
{
	...
	else
	{
		...
		// Setup box data as the forthcoming new P OPC_Pointer
 ...
 *	Node:
 *			- box
 *			- P OPC_Pointer => a node (LSB=0) or a primitive (LSB=1)
 *			- N OPC_Pointer => a node (LSB=0) or a primitive (LSB=1)
 *
...
inline_ void ComputeMinMax(OPC_Point& min, OPC_Point& max, const VertexPointers& vp)
{
...
#define FIND_MAX_VALUES																			\
	/* Get max values */																		\
	OPC_Point CMax(MIN_FLOAT, MIN_FLOAT, MIN_FLOAT);												\
	OPC_Point EMax(MIN_FLOAT, MIN_FLOAT, MIN_FLOAT);												\
...
#define INIT_QUANTIZATION													\
	...
	OPC_Point CQuantCoeff, EQuantCoeff;											\
...
#define PERFORM_QUANTIZATION														\
	...
	{																				\
		...
		OPC_Point Max = Nodes[i].mAABB.mCenter + Nodes[i].mAABB.mExtents;				\
		OPC_Point Min = Nodes[i].mAABB.mCenter - Nodes[i].mAABB.mExtents;				\
		/* For each axis */															\
