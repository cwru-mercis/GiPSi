...
#define __OPC_SPHERECOLLIDER_H__
	...
	struct OPCODE_API SphereCache : VolumeCache
	{
		...
		OPC_Point		Center;		//!< Sphere used when performing the query resulting in cached faces
	...
	class OPCODE_API SphereCollider : public VolumeCollider
	{
		...
		// Internal methods
			...
			// Overlap tests
		inline_				BOOL			SphereContainsBox(const OPC_Point& bc, const OPC_Point& be);
		inline_				BOOL			SphereAABBOverlap(const OPC_Point& center, const OPC_Point& extents);
							BOOL			SphereTriOverlap(const OPC_Point& vert0, const OPC_Point& vert1, const OPC_Point& vert2);
			// Init methods
