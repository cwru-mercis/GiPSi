...
#define __OPC_RAYCOLLIDER_H__
	...
	class OPCODE_API RayCollider : public Collider
	{
		...
		protected:
		// Ray in local space
							OPC_Point			mOrigin;			//!< Ray origin
							OPC_Point			mDir;				//!< Ray direction (normalized)
							OPC_Point			mFDir;				//!< fabsf(mDir)
							OPC_Point			mData, mData2;
		// Stabbed faces
		...
		// Internal methods
			...
			// Overlap tests
		inline_				BOOL			RayAABBOverlap(const OPC_Point& center, const OPC_Point& extents);
		inline_				BOOL			SegmentAABBOverlap(const OPC_Point& center, const OPC_Point& extents);
		inline_				BOOL			RayTriOverlap(const OPC_Point& vert0, const OPC_Point& vert1, const OPC_Point& vert2);
			// Init methods
