...
#define __OPC_OBBCOLLIDER_H__
	...
	class OPCODE_API OBBCollider : public VolumeCollider
	{
		...
		// Precomputed data
							...
							OPC_Point			mTModelToBox;		//!< Translation from model space to obb space
							OPC_Point			mTBoxToModel;		//!< Translation from obb space to model space

							OPC_Point		mBoxExtents;
							OPC_Point		mB0;				//!< - mTModelToBox + mBoxExtents
							OPC_Point		mB1;				//!< - mTModelToBox - mBoxExtents
		...
		// Internal methods
			...
			// Overlap tests
		inline_				BOOL			OBBContainsBox(const OPC_Point& bc, const OPC_Point& be);
		inline_				BOOL			BoxBoxOverlap(const OPC_Point& extents, const OPC_Point& center);
