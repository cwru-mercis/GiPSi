...
#define __OPC_LSSCOLLIDER_H__
	...
	struct OPCODE_API LSSCache : VolumeCache
	{
					LSSCache()
					{
						Previous.mP0 = OPC_Point(0.0f, 0.0f, 0.0f);
						Previous.mP1 = OPC_Point(0.0f, 0.0f, 0.0f);
	...
	class OPCODE_API LSSCollider : public VolumeCollider
	{
		...
		// Internal methods
			...
			// Overlap tests
		inline_				BOOL			LSSContainsBox(const OPC_Point& bc, const OPC_Point& be);
		inline_				BOOL			LSSAABBOverlap(const OPC_Point& center, const OPC_Point& extents);
		inline_				BOOL			LSSTriOverlap(const OPC_Point& vert0, const OPC_Point& vert1, const OPC_Point& vert2);
			// Init methods
