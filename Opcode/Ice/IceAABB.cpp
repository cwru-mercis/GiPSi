...
AABB& AABB::Add(const AABB& aabb)
{
	...
	OPC_Point Min;	GetMin(Min);
	OPC_Point Tmp;	aabb.GetMin(Tmp);
	Min.Min(Tmp);

	OPC_Point Max;	GetMax(Max);
	aabb.GetMax(Tmp);
	Max.Max(Tmp);
...
{
	OPC_Point Ext;	GetExtents(Ext);
	float Max = Ext.Max();

	OPC_Point Cnt;	GetCenter(Cnt);
	cube.SetCenterExtents(Cnt, OPC_Point(Max, Max, Max));
	return Max;
}
...
bool AABB::ComputePlanes(Plane* planes)	const
{
	...
	OPC_Point Center, Extents;
	GetCenter(Center);
	GetExtents(Extents);

	// Writes normals
	planes[0].n = OPC_Point(1.0f, 0.0f, 0.0f);
	planes[1].n = OPC_Point(-1.0f, 0.0f, 0.0f);
	planes[2].n = OPC_Point(0.0f, 1.0f, 0.0f);
	planes[3].n = OPC_Point(0.0f, -1.0f, 0.0f);
	planes[4].n = OPC_Point(0.0f, 0.0f, 1.0f);
	planes[5].n = OPC_Point(0.0f, 0.0f, -1.0f);

	// Compute a OPC_Point on each plane
	OPC_Point p0 = OPC_Point(Center.x+Extents.x, Center.y, Center.z);
	OPC_Point p1 = OPC_Point(Center.x-Extents.x, Center.y, Center.z);
	OPC_Point p2 = OPC_Point(Center.x, Center.y+Extents.y, Center.z);
	OPC_Point p3 = OPC_Point(Center.x, Center.y-Extents.y, Center.z);
	OPC_Point p4 = OPC_Point(Center.x, Center.y, Center.z+Extents.z);
	OPC_Point p5 = OPC_Point(Center.x, Center.y, Center.z-Extents.z);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 *	Computes the aabb OPC_Points.
 *	\param		pts	[out] 8 box OPC_Points
 *	\return		true if success
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool AABB::ComputePoints(OPC_Point* pts)	const
{
	// Checkings
	if(!pts)	return false;
	...
	pts[0] = OPC_Point(min.x, min.y, min.z);
	pts[1] = OPC_Point(max.x, min.y, min.z);
	pts[2] = OPC_Point(max.x, max.y, min.z);
	pts[3] = OPC_Point(min.x, max.y, min.z);
	pts[4] = OPC_Point(min.x, min.y, max.z);
	pts[5] = OPC_Point(max.x, min.y, max.z);
	pts[6] = OPC_Point(max.x, max.y, max.z);
	pts[7] = OPC_Point(min.x, max.y, max.z);
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const OPC_Point* AABB::GetVertexNormals()	const
{
	...
	return (const OPC_Point*)VertexNormals;
}
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const OPC_Point* AABB::GetEdgeNormals() const
{
	...
	return (const OPC_Point*)EdgeNormals;
}
...
const sbyte* AABB::ComputeOutline(const OPC_Point& local_eye, sdword& num)	const
{
	// Get box corners
	OPC_Point min;	GetMin(min);
	OPC_Point max;	GetMax(max);
...
//const OPC_Point&		eye,		//eye OPC_Point (in bbox object coordinates)
//const AABB&			box,		//3d bbox
//const Matrix4x4&	mat,		//free transformation for bbox
//float width, float height, int& num)
float AABB::ComputeBoxArea(const OPC_Point& eye, const Matrix4x4& mat, float width, float height, sdword& num)	const
{
	const sbyte* Outline = ComputeOutline(eye, num);
	if(!Outline)	return -1.0f;

	// Compute box vertices
	OPC_Point vertexBox[8], dst[8];
