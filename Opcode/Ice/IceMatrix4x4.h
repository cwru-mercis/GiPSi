...
//				void	LUDecomposition( sdword* indx, float* d );
		...
		//! Returns a row.
		inline_	void				GetRow(const udword r, OPC_Point& p)	const	{ p.x=m[r][0];	p.y=m[r][1];	p.z=m[r][2];						}
		//! Returns a row.
		...
		//! Sets a row.
		inline_	void				SetRow(const udword r, const OPC_Point& p)		{ m[r][0]=p.x;	m[r][1]=p.y;	m[r][2]=p.z;	m[r][3]= (r!=3) ? 0.0f : 1.0f;		}
		//! Returns a column.
		inline_	void				GetCol(const udword c, HPoint& p)	const	{ p.x=m[0][c];	p.y=m[1][c];	p.z=m[2][c];	p.w=m[3][c];		}
		//! Returns a column.
		inline_	void				GetCol(const udword c, OPC_Point& p)	const	{ p.x=m[0][c];	p.y=m[1][c];	p.z=m[2][c];						}
		//! Sets a column.
		inline_	void				SetCol(const udword c, const HPoint& p)		{ m[0][c]=p.x;	m[1][c]=p.y;	m[2][c]=p.z;	m[3][c]=p.w;		}
		//! Sets a column.
		inline_	void				SetCol(const udword c, const OPC_Point& p)		{ m[0][c]=p.x;	m[1][c]=p.y;	m[2][c]=p.z;	m[3][c]= (c!=3) ? 0.0f : 1.0f;	}

		// Translation
		//! Returns the translation part of the matrix.
		inline_	const HPoint&		GetTrans()							const	{ return GetRow(3);								}
		//! Gets the translation part of the matrix
		inline_	void				GetTrans(OPC_Point& p)					const	{ p.x=m[3][0];	p.y=m[3][1];	p.z=m[3][2];	}
		//! Sets the translation part of the matrix, from a OPC_Point.
		inline_	void				SetTrans(const OPC_Point& p)					{ m[3][0]=p.x;	m[3][1]=p.y;	m[3][2]=p.z;	}
		...
		// Scale
		//! Sets the scale from a OPC_Point. The OPC_Point is put on the diagonal.
		inline_	void				SetScale(const OPC_Point& p)					{ m[0][0]=p.x;	m[1][1]=p.y;	m[2][2]=p.z;	}
		//! Sets the scale from floats. Values are put on the diagonal.
		inline_	void				SetScale(float sx, float sy, float sz)		{ m[0][0]=sx;	m[1][1]=sy;		m[2][2]=sz;		}
		//! Scales from a OPC_Point. Each row is multiplied by a component.
				void				Scale(const OPC_Point& p)
									{
...
/*
		...
		//! Sets a row.
						Matrix4x4&	SetRow(const udword row, const OPC_Point& p)
						{
							m[row][0] = p.x;
							m[row][1] = p.y;
							m[row][2] = p.z;
		...
		//! Sets a column.
						Matrix4x4&	SetCol(const udword col, const OPC_Point& p)
						{
							m[0][col] = p.x;
							m[1][col] = p.y;
							m[2][col] = p.z;
...
*/
		...
		//! Makes a rotation matrix about an arbitrary axis
				Matrix4x4&			Rot(float angle, OPC_Point& p1, OPC_Point& p2);
		...
		//! Inverts the matrix. Determinant must be different from zero, else matrix can't be inverted.
				Matrix4x4&			Invert();
//				Matrix&	ComputeAxisMatrix(OPC_Point& axis, float angle);
		...
		//! Operator for OPC_Point Mul = Matrix4x4 * OPC_Point;
		inline_	OPC_Point				operator*(const OPC_Point& v)		const
									{
										return OPC_Point(	m[0][0]*v.x + m[0][1]*v.y + m[0][2]*v.z + m[0][3],
	...
	inline_ void TransformPoint4x3(OPC_Point& dest, const OPC_Point& source, const Matrix4x4& rot)
	{
	...
	inline_ void TransformPoint3x3(OPC_Point& dest, const OPC_Point& source, const Matrix4x4& rot)
	{
