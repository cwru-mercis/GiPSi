...
#define __ICEMATRIX3X3_H__
	...
	class ICEMATHS_API Matrix3x3
	{
		...
		inline_	void			Scale(const OPC_Point& p)
								{
		...
		//! Returns a row.
		inline_	const OPC_Point&	GetRow(const udword r)				const	{ return *(const OPC_Point*)&m[r][0];	}
		//! Returns a row.
		inline_	OPC_Point&			GetRow(const udword r)						{ return *(OPC_Point*)&m[r][0];			}
		//! Sets a row.
		inline_	void			SetRow(const udword r, const OPC_Point& p)		{ m[r][0] = p.x;	m[r][1] = p.y;	m[r][2] = p.z;	}
		//! Returns a column.
		inline_	void			GetCol(const udword c, OPC_Point& p)	const	{ p.x = m[0][c];	p.y = m[1][c];	p.z = m[2][c];	}
		//! Sets a column.
		inline_	void			SetCol(const udword c, const OPC_Point& p)		{ m[0][c] = p.x;	m[1][c] = p.y;	m[2][c] = p.z;	}
		...
		inline_	void			SkewSymmetric(const OPC_Point& a)
								{
									m[0][0] = 0.0f;
									m[0][1] = -a.z;
									m[0][2] = a.y;
		...
		//! Makes a rotation matrix mapping vector "from" to vector "to".
				Matrix3x3&		FromTo(const OPC_Point& from, const OPC_Point& to);
		...
		//! Make a rotation matrix about an arbitrary axis
				Matrix3x3&		Rot(float angle, const OPC_Point& axis);
...
void FromQuatL2(const Quat &q, float l2);
		...
		//! Operator for OPC_Point Mul = Matrix3x3 * OPC_Point;
		inline_	OPC_Point			operator*(const OPC_Point& v)		const		{ return OPC_Point(GetRow(0)|v, GetRow(1)|v, GetRow(2)|v); }
		...
		inline_	const OPC_Point&	operator[](int row)		const	{ return *(const OPC_Point*)&m[row][0];	}
		inline_	OPC_Point&			operator[](int row)				{ return *(OPC_Point*)&m[row][0];		}
