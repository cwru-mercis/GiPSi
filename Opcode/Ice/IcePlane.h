...
#define __ICEPLANE_H__
	...
	class ICEMATHS_API Plane
	{
		...
		//! Constructor from a OPC_Point on the plane and a normal
		inline_			Plane(const OPC_Point& p, const OPC_Point& n)							{ Set(p, n);									}
		//! Constructor from three OPC_Points
		inline_			Plane(const OPC_Point& p0, const OPC_Point& p1, const OPC_Point& p2)		{ Set(p0, p1, p2);								}
		//! Constructor from a normal and a distance
		inline_			Plane(const OPC_Point& _n, float _d)								{ n = _n; d = _d;								}
		//! Copy constructor
		...
		inline_	Plane&	Set(const OPC_Point& p, const OPC_Point& _n)							{ n = _n; d = - p | _n;			return *this;	}
				Plane&	Set(const OPC_Point& p0, const OPC_Point& p1, const OPC_Point& p2);

		inline_	float	Distance(const OPC_Point& p)			const						{ return (p | n) + d;							}
		inline_	bool	Belongs(const OPC_Point& p)				const						{ return fabsf(Distance(p)) < PLANE_EPSILON;	}
		...
		public:
		// Members
				OPC_Point	n;		//!< The normal to the plane
				float	d;		//!< The distance from the origin

		// Cast operators
		inline_			operator OPC_Point()					const						{ return n;										}
		...
		inline_	Plane&	operator*=(const Matrix4x4& m)
						{
							// Old code from Irion. Kept for reference.
							OPC_Point n2 = HPoint(n, 0.0f) * m;
							d = -((OPC_Point) (HPoint( -d*n, 1.0f ) * m) | n2);
							n = n2;
							return *this;
						}
	};
	...
	inline_	void TransformPlane(Plane& transformed, const Plane& plane, const Matrix4x4& transform)
	{
		...
		// Compute new d
		transformed.d = plane.d - (OPC_Point(transform.GetTrans())|transformed.n);
	}
	...
	inline_	void TransformPlane(Plane& plane, const Matrix4x4& transform)
	{
		...
		// Compute new d
		plane.d -= OPC_Point(transform.GetTrans())|plane.n;
	}
