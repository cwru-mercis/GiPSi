...
#define AABB_COMMON_METHODS																											\
			...
			const sbyte*	ComputeOutline(const OPC_Point& local_eye, sdword& num)												const;	\
			float			ComputeBoxArea(const OPC_Point& eye, const Matrix4x4& mat, float width, float height, sdword& num)	const;	\
			bool			IsInside(const AABB& box)																		const;	\
			bool			ComputePlanes(Plane* planes)																	const;	\
			bool			ComputePoints(OPC_Point* pts)																		const;	\
			const OPC_Point*	GetVertexNormals()																				const;	\
			const udword*	GetEdges()																						const;	\
			const OPC_Point*	GetEdgeNormals()																				const;	\
	inline_	BOOL			ContainsPoint(const OPC_Point& p)																	const	\
							{																										\
	...
	class ICEMATHS_API AABB
	{
		 ...
		 *	\param		min			[in] the min OPC_Point
		 *	\param		max			[in] the max OPC_Point
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetMinMax(const OPC_Point& min, const OPC_Point& max)		{ mMin = min;		mMax = max;									}

		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/**
		 *	Setups an AABB from center & extents vectors.
		 *	\param		c			[in] the center OPC_Point
		 *	\param		e			[in] the extents vector
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetCenterExtents(const OPC_Point& c, const OPC_Point& e)	{ mMin = c - e;		mMax = c + e;								}
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetEmpty()											{ OPC_Point p(MIN_FLOAT, MIN_FLOAT, MIN_FLOAT);	mMin = -p; mMax = p;}

		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/**
		 *	Setups a OPC_Point AABB.
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetPoint(const OPC_Point& pt)							{ mMin = mMax = pt;												}
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						float		GetSize()								const		{ OPC_Point e; GetExtents(e);	return e.Max();	}

		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		/**
		 *	Extends the AABB.
		 *	\param		p	[in] the next OPC_Point
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		Extend(const OPC_Point& p)
									{
		...
		//! Get min OPC_Point of the box
		inline_			void		GetMin(OPC_Point& min)						const		{ min = mMin;								}
		//! Get max OPC_Point of the box
		inline_			void		GetMax(OPC_Point& max)						const		{ max = mMax;								}

		//! Get component of the box's min OPC_Point along a given axis
		inline_			float		GetMin(udword axis)						const		{ return mMin[axis];						}
		//! Get component of the box's max OPC_Point along a given axis
		inline_			float		GetMax(udword axis)						const		{ return mMax[axis];						}

		//! Get box center
		inline_			void		GetCenter(OPC_Point& center)				const		{ center = (mMax + mMin)*0.5f;				}
		//! Get box extents
		inline_			void		GetExtents(OPC_Point& extents)				const		{ extents = (mMax - mMin)*0.5f;				}
		...
		//! Get box diagonal
		inline_			void		GetDiagonal(OPC_Point& diagonal)			const		{ diagonal = mMax - mMin;					}
		...
		inline_			AABB&		operator*=(float s)
									{
										OPC_Point Center;	GetCenter(Center);
										OPC_Point Extents;	GetExtents(Extents);
		...
		//! Operator for AABB += OPC_Point. Translates the box.
		inline_			AABB&		operator+=(const OPC_Point& trans)
									{
										mMin+=trans;
										mMax+=trans;
										return *this;
		...
		private:
						Point		mMin;			//!< Min OPC_Point
						Point		mMax;			//!< Max OPC_Point
	};
	...
	class ICEMATHS_API AABB
	{
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetMinMax(const OPC_Point& min, const OPC_Point& max)		{ mCenter = (max + min)*0.5f; mExtents = (max - min)*0.5f;		}
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetCenterExtents(const OPC_Point& c, const OPC_Point& e)	{ mCenter = c;	 mExtents = e;									}
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		SetPoint(const OPC_Point& pt)							{ mCenter = pt; mExtents.Zero();								}
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void		Extend(const OPC_Point& p)
									{
										OPC_Point Max = mCenter + mExtents;
										OPC_Point Min = mCenter - mExtents;
		...
		//! Get min OPC_Point of the box
		inline_			void		GetMin(OPC_Point& min)						const		{ min = mCenter - mExtents;					}
		//! Get max OPC_Point of the box
		inline_			void		GetMax(OPC_Point& max)						const		{ max = mCenter + mExtents;					}
		...
		//! Get box center
		inline_			void		GetCenter(OPC_Point& center)				const		{ center = mCenter;							}
		//! Get box extents
		inline_			void		GetExtents(OPC_Point& extents)				const		{ extents = mExtents;						}
		...
		//! Get box diagonal
		inline_			void		GetDiagonal(OPC_Point& diagonal)			const		{ diagonal = mExtents * 2.0f;				}
		...
		inline_			bool		GomezIntersect(const AABB& a)
									{
										OPC_Point	T = mCenter - a.mCenter;	// Vector from A to B
		...
		inline_			void		Rotate(const Matrix4x4& mtx, AABB& aabb)	const
									{
										...
										OPC_Point Ex(mtx.m[0][0] * mExtents.x, mtx.m[0][1] * mExtents.x, mtx.m[0][2] * mExtents.x);
										IR(Ex.x)&=0x7fffffff;	IR(Ex.y)&=0x7fffffff;	IR(Ex.z)&=0x7fffffff;

										OPC_Point Ey(mtx.m[1][0] * mExtents.y, mtx.m[1][1] * mExtents.y, mtx.m[1][2] * mExtents.y);
										IR(Ey.x)&=0x7fffffff;	IR(Ey.y)&=0x7fffffff;	IR(Ey.z)&=0x7fffffff;

										OPC_Point Ez(mtx.m[2][0] * mExtents.z, mtx.m[2][1] * mExtents.z, mtx.m[2][2] * mExtents.z);
		...
		private:
						OPC_Point		mCenter;			//!< AABB Center
						OPC_Point		mExtents;			//!< x, y and z extents
	};

#endif

	inline_ void ComputeMinMax(const OPC_Point& p, OPC_Point& min, OPC_Point& max)
	{
		if(p.x > max.x)	max.x = p.x;
		if(p.x < min.x)	min.x = p.x;
	...
	inline_ void ComputeAABB(AABB& aabb, const OPC_Point* list, udword nb_pts)
	{
		if(list)
		{
			OPC_Point Maxi(MIN_FLOAT, MIN_FLOAT, MIN_FLOAT);
			OPC_Point Mini(MAX_FLOAT, MAX_FLOAT, MAX_FLOAT);
			while(nb_pts--)
			{
