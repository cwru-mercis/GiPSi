 ...
 *
 *		If P0 and P1 are two 3D OPC_Points, let's define:
 *		- d = distance between P0 and P1
 *		- Origin	= P0
 *		- Direction	= (P1 - P0) / d = normalized direction vector
 *		- A parameter t such as a OPC_Point P on the line (P0,P1) is P = Origin + t * Direction
 *		- t = 0  -->  P = P0
 *		- t = d  -->  P = P1
 *
...
#define SEGMENT_PRIM(prim_index, flag)														\
	...
	{																						\
		/* Intersection OPC_Point is valid if dist < segment's length */						\
...
BOOL RayCollider::InitQuery(const Ray& world_ray, const Matrix4x4* world, udword* face_id)
{
	...
	if(TemporalCoherenceEnabled() && FirstContactEnabled() && face_id && *face_id!=INVALID_ID)
	{
...
#endif
		{
			...
			if(RayTriOverlap(*VP.Vertex[0], *VP.Vertex[1], *VP.Vertex[2]))
			{
				// Intersection OPC_Point is valid if:
				// - distance is positive (else it can just be a face behind the orig OPC_Point)
...
void RayCollider::_SegmentStab(const AABBQuantizedNode* node)
{
	...
	const OPC_Point Center(float(Box.mCenter[0]) * mCenterCoeff.x, float(Box.mCenter[1]) * mCenterCoeff.y, float(Box.mCenter[2]) * mCenterCoeff.z);
	const OPC_Point Extents(float(Box.mExtents[0]) * mExtentsCoeff.x, float(Box.mExtents[1]) * mExtentsCoeff.y, float(Box.mExtents[2]) * mExtentsCoeff.z);
