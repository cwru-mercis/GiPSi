...
static float OPC_PointTriangleSqrDist(const OPC_Point& point, const OPC_Point& p0, const OPC_Point& p1, const OPC_Point& p2)
{
	// Hook
	OPC_Point TriEdge0 = p1 - p0;
	OPC_Point TriEdge1 = p2 - p0;

	OPC_Point kDiff	= p0 - point;
...
{
	// Hook
	OPC_Point rkSeg0Direction	= rkSeg0.ComputeDirection();
	OPC_Point rkSeg1Direction	= rkSeg1.ComputeDirection();

	OPC_Point kDiff	= rkSeg0.mP0 - rkSeg1.mP0;
		...
		if(fS >= 0.0f)
		{
			if(fS <= fDet)
			{
				if(fT >= 0.0f)
				{
					...
					{
						// minimum at two interior OPC_Points of 3D lines
						float fInvDet = 1.0f/fDet;
						fS *= fInvDet;
						fT *= fInvDet;
						fSqrDist = fS*(fA00*fS+fA01*fT+2.0f*fB0) + fT*(fA01*fS+fA11*fT+2.0f*fB1)+fC;
...
static float OPC_SegmentTriangleSqrDist(const Segment& segment, const OPC_Point& p0, const OPC_Point& p1, const OPC_Point& p2)
{
	// Hook
	const OPC_Point TriEdge0 = p1 - p0;
	const OPC_Point TriEdge1 = p2 - p0;

	const OPC_Point& rkSegOrigin	= segment.GetOrigin();
	OPC_Point rkSegDirection		= segment.ComputeDirection();

	OPC_Point kDiff = p0 - rkSegOrigin;
...
inline_ BOOL LSSCollider::LSSTriOverlap(const OPC_Point& vert0, const OPC_Point& vert1, const OPC_Point& vert2)
{
	// Stats
	mNbVolumePrimTests++;
